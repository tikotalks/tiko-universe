 ...
 * `workers/sentence-api/data`, not against invented fixtures: if a tile's text
...
function repoRoot(): string {
  ...
  for (let depth = 0; depth < 8; depth += 1) {
    if (existsSync(resolve(directory, 'workers/sentence-api/data'))) return directory
    const parent = dirname(directory)
    if (parent === directory) break
    directory = parent
  }
  throw new Error('could not locate workers/sentence-api/data from the working directory')
}

function loadPack(language: string): Map<string, SelectedWord> {
  const path = resolve(repoRoot(), `workers/sentence-api/data/${language}-v1.json`)
