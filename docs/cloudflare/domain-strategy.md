...
| --- | --- | --- | --- |
| identity-api | `tiko-identity-api` | `id.tiko.mt/*` | `id.tiko.mt` |
| app-api | `tiko-app-api` | `api.tikoapi.org/v1/apps/*` | — (path route) |
| generation-api | `tiko-generation-api` | `api.tikoapi.org/v1/generation/*` | — (path route) |
| content-api | `tiko-content-api` | `api.tikoapi.org/v1/content/*` | — (path route) |

### Dedicated subdomain workers

| Worker | Worker Name | Route Pattern | Custom Domain |
| --- | --- | --- | --- |
| media-api | `tiko-media-api` | `media.tikoapi.org/*` | `media.tikoapi.org` |
| admin-api | `tiko-admin-api` | `admin.tikoapi.org/*` | `admin.tikoapi.org` |
| tts-api (temp) | `tiko-tts-api` | `tts.tikoapi.org/*` | `tts.tikoapi.org` |
...
| Worker | Dev Route |
| --- | --- |
| identity-api | `dev.id.tiko.mt/*` |
| app-api | `dev.api.tikoapi.org/v1/apps/*` |
| generation-api | `dev.api.tikoapi.org/v1/generation/*` |
| media-api | `media.tikoapi.org/*` (uses `-dev` suffix worker name) |
| content-api | `dev.api.tikoapi.org/v1/content/*` |
| admin-api | `dev.admin.tikoapi.org/*` |
| tts-api (temp) | `dev.tts.tikoapi.org/*` |
...
routes = [
  { pattern = "api.tikoapi.org/v1/apps*", zone_name = "tikoapi.org" }
]

# Custom domain (dedicated subdomain)
custom_domains = ["media.tikoapi.org"]
```
...
| --- | --- | --- |
| `IDENTITY_BASE_URL` | `https://api.tikoapi.org/v1` | app-api, admin-api, media-api, shared auth |
| `APP_API_URL` | `https://api.tikoapi.org/v1/apps` | admin-api |
| `GENERATION_API_URL` | `https://api.tikoapi.org/v1/generation` | admin-api |
| `MEDIA_API_URL` | `https://media.tikoapi.org/v1` | admin-api |
| `MAGIC_LINK_BASE_URL` | `https://tikoapi.org/identity/magic` | identity-api |
