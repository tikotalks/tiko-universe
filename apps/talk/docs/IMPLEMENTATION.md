...
- [x] **Bundle the real packs** (`@tiko/talk-packs`) — all 54 languages ship with the app, one chunk each, and the board is built from them rather than fetched. There is no reduced offline mode.
...
12. **`@tiko/talk-packs`** — the packs themselves, shared with the worker and the seed generator
